...
Cloudflare 会为本次安装创建 GitHub 仓库，自动创建 Worker、D1 数据库和 R2 bucket，要求用户设置初始登录密码 Secret，执行 migration，并连接 Workers Builds。生成的仓库每天检查一次 EdgeEver 上游更新：默认 `stable` 通道跟随正式 Release，可选 `edge` 通道跟随上游 `main`。

配置、验证、更新通道和排错说明见[使用 Cloudflare 一键部署 EdgeEver](docs/deploy-cloudflare-button.zh-CN.md)。
